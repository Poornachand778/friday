...
Output: "Pre-production is your best friend - plan every shot, every prop, every sandwich for the crew. Trust me, hungry technicians are scarier than any villain you can write!"
```
...
## 🔄 **Iteration 2 Progress Snapshot**

### **WhatsApp Data Pipeline**

- `src/data_processing/ingest/whatsapp_ingest.py` unpacks zipped exports, normalizes encodings, builds `data/inventory/whatsapp_index.json`, and logs placeholders into `memory/data/sft_datasets.jsonl`.
- `scripts/whatsapp_to_sft.py` turns chat transcripts into SFT-ready JSONL, scrubs PII, tags persona-specific prompts, and optionally transliterates roman Telugu back to script.

### **Memory System Tooling**

- `scripts/memory_cli.py` exposes CRUD helpers for persona, long-term memories, snippets, and STM sessions in the JSONL store.
- `scripts/extract_memories.py` mines WhatsApp replies for durable guardrails and reusable templates with auto-tagging.
- `scripts/embed_content.py` provides a deterministic hashing-based embedding fallback so we can vectorize memories without external services.

### **Evaluation & Scene Ops**

- `scripts/run_iter2_smoke.py` adds a scripted evaluator that composes heuristic Friday responses and logs the metadata needed for iteration tracking.
- `mcp/scene_manager/service.py` centralizes scene retrieval, embedding, and revision updates so both the FastAPI endpoint and MCP worker stay in sync.

### **Data & Persona Refresh**

- Persona beliefs now mix Telugu-English guardrails; screenplay drafts live under `data/film/scripts/`, and iteration 1 JSONL datasets include cached word-frequency and unicode audit artifacts.
- New `data/{inventory,processed,raw,sft}` and `memory/` assets capture raw exports, processed chats, evaluation runs, and SFT bookkeeping.

### **Dependencies**

- `requirements.txt` brings in `sentence-transformers`, `wordfreq`, `fastapi`, `uvicorn`, and other helpers required by the ingestion, embedding, and service layers.
