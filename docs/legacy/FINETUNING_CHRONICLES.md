...
# Part 2: Iteration 2 - The Clean Data Saga

---

## 📺 Episode 17: The Transliteration Discovery _(January 2026)_

_"When Training Data Contains Hidden Patterns You Didn't Want"_

Months after Iteration 1's success, we returned for Iteration 2 with a larger dataset. But a careful analysis revealed a hidden problem:

### The Problem

The WhatsApp data from Iteration 1 had **transliteration** mixed in - Telugu written in English characters:
- "Enti Boss" instead of "ఏంటి Boss"
- "Chala bagundi" instead of "చాలా బాగుంది"

This was training Friday to respond with transliterated Telugu, which was NOT the desired behavior. We wanted natural code-switching between pure Telugu script and English.

### The Solution: Clean Dataset Strategy

Instead of trying to fix the transliteration, we:
1. **Excluded** all transliterated examples
2. **Built a clean dataset** from scratch with 768 examples
3. **Sources**: 700 WhatsApp (filtered) + 33 tool examples + 35 decision scenarios
4. **Result**: Natural code-switching only, no transliteration

---

## 📺 Episode 18: The AWS Account Migration _(January 10, 2026)_

_"When Credits Matter More Than Familiarity"_

### The Challenge

- Old AWS account: No credits remaining
- New AWS account (473398167035): $200 in credits waiting to be used
- Had to migrate credentials, roles, and S3 setup

### The Journey

1. **Credential Update**: Updated .env with new AWS credentials
2. **Role Creation**: Created new `FridayAI-SageMaker-ExecutionRole` with proper trust policy
3. **Quota Check**: Discovered ml.g5.2xlarge quota = 0, but ml.g5.12xlarge available
4. **Configuration Migration**: All settings transferred successfully

### Key Fix: Trust Policy

The new role needed sagemaker.amazonaws.com in its trust policy:
```json
{
  "Version": "2012-10-17",
  "Statement": [{
...
## 📺 Episode 19: The Multi-GPU Memory Battle _(January 10, 2026)_

_"When Chronicles Wisdom Saves the Day"_

### Attempt 1: batch_size=4, grad_acc=16

**Result**: OOM on `parallel_apply.py`

### Attempt 2: batch_size=2, grad_acc=32

**Result**: Still OOM!

### The Revelation

Reading our own FINETUNING_CHRONICLES.md revealed the answer from Iteration 1:
- **Multi-GPU QLoRA requires `device_map="auto"`**, not `{"": local_rank}`
- **Batch size = 1 per GPU** is the proven configuration
- **Smart GPU distribution** instead of replication
...
## 📺 Episode 20: Victory - Iteration 2 Complete! _(January 10, 2026)_

_"The Chronicles-Validated Configuration Delivers"_

### Final Training Configuration

| Parameter | Value |
|-----------|-------|
| Dataset | 768 examples (clean, no transliteration) |
| Base Model | meta-llama/Meta-Llama-3.1-8B-Instruct |
| Instance | ml.g5.12xlarge (4x A10G GPUs) |
| Quantization | QLoRA 4-bit (NF4) |
| Device Map | "auto" (smart distribution) |
| Batch Size | 1 per GPU |
| Grad Accumulation | 64 steps |
| Max Seq Length | 1536 tokens |
| Epochs | 3 |
| LoRA Rank | 16 |
| LoRA Alpha | 32 |

### Training Job: `friday-lora-20260110-022927`
...
💰 Cost: ~$6.20
📦 Adapter Size: 167MB
🎯 Model: s3://sagemaker-us-east-1-473398167035/friday-lora-20260110-022927/output/model.tar.gz
```

### What Made It Work

1. **Chronicles wisdom**: Reading our own documentation saved hours
2. **Clean data**: No transliteration = cleaner training signal
3. **QLoRA + device_map="auto"**: The proven multi-GPU formula
4. **Conservative batch size**: 1 per GPU with high accumulation
...
|--------|-------------|-------------|
| Total Examples | 242 | 768 |
| WhatsApp Data | 0 | 700 |
| Tool Examples | 12 | 33 |
| Transliteration | Present | Removed |
| Code-switching | Partial | Natural only |

### Technical Configuration

| Setting | Iteration 1 | Iteration 2 |
|---------|-------------|-------------|
| Instance | ml.g5.12xlarge | ml.g5.12xlarge |
| device_map | "auto" | "auto" |
| Batch Size | 1 | 1 |
| Grad Acc | 8 | 64 |
| Max Seq Len | 2048 | 1536 |

### Cost Comparison

| Iteration | Duration | Cost |
|-----------|----------|------|
| 1 | ~4 min | ~$0.60 |
| 2 | ~41 min | ~$6.20 |

The longer training time for Iteration 2 reflects:
- 3x more examples (768 vs 242)
- Higher gradient accumulation (64 vs 8)
- More thorough training with larger dataset
...
Removing transliterated data improved training signal clarity, even though it reduced the raw example count.

### 2. Documentation is Your Best Friend

Our own chronicles from Iteration 1 provided the exact configuration needed for multi-GPU success.

### 3. Cloud Credits Strategy

Having multiple AWS accounts with different credit pools allows more training iterations without financial stress.

### 4. Conservative Memory Settings Win

For multi-GPU QLoRA:
- batch_size=1 per GPU (not 2 or 4)
- device_map="auto" (not per-rank mapping)
- Higher gradient accumulation compensates
...
- [ ] Test Iteration 2 model with Streamlit tester
- [ ] Compare responses: Iteration 1 vs Iteration 2
- [ ] Validate code-switching quality

### Milestone 2: Local Inference

- [ ] Set up local hardware (RTX 4090 recommended)
- [ ] Deploy model with vLLM or llama.cpp
- [ ] Create voice pipeline integration

### Milestone 3: Voice MVP

- [ ] Integrate Whisper for speech-to-text
- [ ] Add TTS for Friday's responses
- [ ] Implement wake word detection

---

_"The journey continues. Iteration 2 is done, but Friday AI's evolution has just begun."_

**Date**: January 10, 2026
**Status**: Iteration 2 Complete
**Next**: Local Inference & Voice MVP
